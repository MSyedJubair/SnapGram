    ...
    <section className='z-50 flex justify-between items-end w-full sticky bottom-0 h-screen rounded-4xl bg-dark-2 px-5 py-4 md:hidden'>
      {
        bottombarLinks.map((link) => {
          ...
          return (
            <Link key={link.label} to={link.route} className={`flex flex-col justify-center rounded-lg p-3 base-medium hover:bg-[#877EFF] transition group ${isActive && "bg-[#877EFF]"} `}>
