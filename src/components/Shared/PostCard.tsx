import { useUserContext } from "@/context/AuthContext";
import { multiFormatDateString } from "@/lib/utils";
import { Link } from "react-router-dom";
import PostStats from "./PostStats";
import type { IPost } from "@/types";

const PostCard = ({ post }: { post: IPost }) => {
  const { user } = useUserContext();

  if (!post.Creator) return null;

  return (
    <div className="bg-dark-2 rounded-3xl border border-dark-4 p-5 lg:p-7 w-full max-w-screen-sm transition-shadow hover:shadow-2xl hover:shadow-black/20">
      <div className="flex justify-between items-center mb-5">
        <div className="flex items-center gap-3">
          <Link to={`/profile/${post.Creator}`}>
            <img
              src={user?.imageUrl || "/assets/icons/profile-placeholder.svg"}
              alt="creator"
              className="rounded-full w-12 h-12 object-cover border-2 border-primary-500"
            />
          </Link>

          <div className="flex flex-col">
            <p className="base-medium lg:body-bold text-light-1">
              {user.name}
            </p>
            <div className="flex-center gap-2 text-light-3">
              <p className="subtle-semibold lg:small-regular">
                {multiFormatDateString(post.$createdAt)}
              </p>
              {post.location && (
                <>
                  <span className="w-1 h-1 rounded-full bg-light-3 opacity-50" />
                  <p className="subtle-semibold lg:small-regular line-clamp-1">
                    {post.location}
                  </p>
                </>
              )}
            </div>
          </div>
        </div>

        <Link
          to={`/update-post/${post.$id}`}
          className={`${user.id !== post.Creator && "hidden"}`}
        >
          <img src="/assets/icons/edit.svg" alt="edit" width={20} height={20} className="hover:opacity-70 transition-opacity" />
        </Link>
      </div>

      <Link to={`/posts/${post.$id}`}>
        <div className="small-medium lg:base-medium py-5">
          <p>{post.Caption}</p>
          <ul className="flex gap-1 mt-2">
            {post.Tags.map((tag: string) => (
              <li key={tag} className="text-light-3">
                #{tag}
              </li>
            ))}
          </ul>
        </div>

        <img
          src={post.imageURL?.toString() || "/assets/icons/profile-placeholder.svg"}
          alt="post image"
          className="h-64 xs:h-[400px] lg:h-[450px] w-full rounded-[24px] object-cover mb-5"
        />
      </Link>

      <PostStats post={post} userId={user.id} />
    </div>
  );
};

export default PostCard;
